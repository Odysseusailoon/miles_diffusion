...
Launched by ``test_attention_parity.py``, not discovered as a CI test itself.

    full Q/K/V [B, S, H, D], identical on every rank (seeded + broadcast)
        |
        |-- reference: one full-sequence attention call with the selected kernel
        |       -> out, dQ, dK, dV   (sliced to this rank's S/4 for comparison)
        |
        `-- USP: rank r keeps shard r of S, runs usp_attention over
                 ulysses x ring = 4x1 | 2x2 | 1x4, backward with the matching
                 grad_output shard -> local out, dQ, dK, dV

    parity mode        : S=128; 4x1 must be bitwise (Ulysses only permutes heads/sequence),
                         ring splits merge in fp32 and are held to TOLERANCES
    deterministic mode : S=8192 so every local KV span covers several FA3 backward tiles;
                         run USP twice under torch.use_deterministic_algorithms(True),
                         everything must be bitwise identical

``--attention-backend`` picks the kernel family for reference, USP local attention
and ring steps alike: native flash (default), ``_native_cudnn`` or ``_flash_3``.
"""
...
SP_SIZE = 4
SHAPE = (2, 128, 8, 64)  # [batch, global sequence, heads, head dim]; sequence set from --sequence-length
...
def _local_attention(query, key, value):
    ...
    if _ATTENTION_BACKEND == "_flash_3":
        return flash_attention_3.flash3_attention(query, key, value, scale=scale)
...
def main():
    global _ATTENTION_BACKEND, SHAPE
    ...
    parser.add_argument("--sequence-length", type=int, default=SHAPE[1])
    parser.add_argument("--deterministic", action="store_true")
    args = parser.parse_args()

    _ATTENTION_BACKEND = args.attention_backend
    SHAPE = (SHAPE[0], args.sequence_length, *SHAPE[2:])
