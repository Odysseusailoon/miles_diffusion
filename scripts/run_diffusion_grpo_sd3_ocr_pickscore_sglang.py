"""SD3.5-medium GRPO on 0.8 OCR + 0.2 PickScore through the sglang-diffusion /rollout/generate path.

The OCR recipe with PickScore mixed in via ``weighted_mixture_rm`` (the weighting Stepwise-Flow-GRPO
uses for its OCR task) and prompts shuffled per epoch; every other flag matches the OCR recipe.
...
Usage:
    ...
    MILES_SCRIPT_DEBUG_ALIGNMENT=1 python3 scripts/run_diffusion_grpo_sd3_ocr_pickscore_sglang.py
"""
...
def execute(args: ScriptArgs, data_dir: str) -> None:
    run_name = f"diffusion_grpo_sd3_ocr_pickscore_sglang_{U.create_run_id()}"
