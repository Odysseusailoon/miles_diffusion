"""Qwen-Image 2.1 PickScore Flow-GRPO, 1-GPU colocate smoke / alignment recipe.

CFG is distilled into the checkpoint (true_cfg_scale=1.0). The SDE window
mirrors the Qwen-Image 1.0 flow_grpo pickscore recipe: 10 steps, train 3-4.

Default topology fits a single 32 GB card for a wiring / log_prob_diff check.
A production run should raise batch, group size, and resolution, and add GPUs.

Requires:
  * sglang with Qwen-Image 2.1 (https://github.com/sgl-project/sglang/pull/39983)
  * diffusers with QwenImage21Transformer2DModel (https://github.com/huggingface/diffusers/pull/14804)
...
Usage:
    ...
    # local dummy / private checkpoint
    python3 scripts/run_diffusion_grpo_qwenimage21_pickscore_1gpu.py \\
        --hf-checkpoint /path/to/qwen-image-2.1 --prompt-data /path/to/pickscore
"""
...
def execute(args: ScriptArgs, data_dir: str) -> None:
    run_name = f"diffusion_grpo_qwenimage21_pickscore_1gpu_{U.create_run_id()}"
    ...
    wandb_args = U.get_default_wandb_args(
        __file__, run_id=run_name, project=WANDB_PROJECT, wandb_log_num_images=4, wandb_log_image_interval=1
    )
    ...
    perf_args = "--gradient-checkpointing --micro-batch-size-sample 1 --micro-batch-size-tstep 1 "
