"""Qwen-Image 2.1 PickScore Flow-GRPO, 1-GPU long run.

Mirrors the Qwen-Image 1.0 flow_grpo recipe (400 rollouts, 512², 10 steps,
SDE window 3–5, LoRA 64/128, save every 10) but scaled to one colocated GPU.

1.0 / 5-GPU reference: 32 prompts × 16 samples, 2 opt steps, micro-batch 8.
This 1-GPU cut: 8 prompts × 8 samples = 64 / rollout, 2 opt steps of 32,
micro-batch 2. CFG stays 1.0 — 2.1 is distilled.

Usage:
    python3 scripts/run_diffusion_grpo_qwenimage21_pickscore_1gpu_long.py \\
...
def execute(args: ScriptArgs, data_dir: str) -> None:
    run_name = f"diffusion_grpo_qwenimage21_pickscore_1gpu_long_{U.create_run_id()}"
    ...
    perf_args = "--gradient-checkpointing --micro-batch-size-sample 2 --micro-batch-size-tstep 1 "
