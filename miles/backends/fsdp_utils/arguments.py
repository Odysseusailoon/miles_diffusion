...
#   _flash_3            (dense FA3) : trainer-owned binding (flash_attention_3.py) follows torch's flag
#   flash* (other)      (flash-attn): patch deterministic= on diffusers' entry points (flag can't reach it)
#   sage / xformers / flex / aiter  : opaque to torch, no hook -> reject (validate)

FLASH3_BACKEND = "_flash_3"

# diffusers' module globals for flash-attn 2 and FA3 varlen; dense FA3 is bound in flash_attention_3.py
...
def is_flash3_backend(backend) -> bool:
    # diffusers' AttentionBackendName is a str enum, so this also accepts the enum member
    return backend == FLASH3_BACKEND
